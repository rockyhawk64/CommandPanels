...
public class AddPlaceholderTag implements TagResolver {

    @Override
    public boolean handle(Context ctx, Panel panel, PanelPosition pos, Player player, String command) {
        if (!command.startsWith("add-placeholder=")) {
            return false;
        }

        String cmd = command.substring("add-placeholder=".length()).trim();
        if (panel == null) {
            return true;
        }
        ...
        for (int i = 0; i < cm.length; i++) {
            if (cm[i].equals('[')) {
                ...
                if (!panel.placeholders.keys.containsKey(placeholder)) {
